...
class OFDMModel:
    def __init__(self, num_subcarriers, num_symbols, num_antennas, cyclic_prefix_length, channel_type, modulation_type):
    ...
    def apply_channel(self, signal):
        ...
        # Add cyclic prefix
        output_signal_cp = np.zeros((self.num_subcarriers, self.num_symbols + self.cyclic_prefix_length, self.num_antennas), dtype=complex)
        output_signal_cp[:, :self.cyclic_prefix_length, :] = output_signal[:, -self.cyclic_prefix_length:, :]
        output_signal_cp[:, self.cyclic_prefix_length:, :] = output_signal

        return output_signal_cp
    ...
    def equalize_signal(self, signal):
        ...
        signal_no_cp = signal[:, self.cyclic_prefix_length:, :]

        equalized_signal = np.zeros_like(signal_no_cp)
        for i in range(self.num_subcarriers):
            for j in range(self.num_symbols):
                channel_inv = np.linalg.inv(self.channel_matrix[i])
                equalized_signal[i, j] = np.dot(channel_inv, signal_no_cp[i, j])
